# Mesh image-based ship: Git commit → Build → Digest → recreate → self-check → smoke
#
...
#   2) Pack build context → remote
#   3) docker build -t geekpark-mesh:<tag> --build-arg GIT_SHA=...
#   4) compose up -d --force-recreate mesh + worker
#   5) Wait REPRO_STATUS=PASS (/api/repro/status) — hard gate
#   6) Smoke + emit ENV_REPRO_BASELINE
#
...
export MESH_EXPECTED_GIT_SHA=`$SHORT
...
grep -q '^MESH_IMAGE_DIGEST=' .env 2>/dev/null && sed -i "s|^MESH_IMAGE_DIGEST=.*|MESH_IMAGE_DIGEST=`$DIGEST|" .env || echo "MESH_IMAGE_DIGEST=`$DIGEST" >> .env
grep -q '^MESH_IMAGE_ID=' .env 2>/dev/null && sed -i "s|^MESH_IMAGE_ID=.*|MESH_IMAGE_ID=`$ID|" .env || echo "MESH_IMAGE_ID=`$ID" >> .env

echo "==> compose up (file=`$COMPOSE) image=`$TAG (force-recreate)"
test -n "`$COMPOSE"
test -f "`$COMPOSE"
...
for i in `$(seq 1 36); do
  ...
  if [ "`$STATUS" = "FAIL" ]; then
    echo "`$BODY"
    echo "REPRO_SELFCHECK_FAIL" >&2
    exit 2
  fi
  sleep 2
done
if [ "`$REPRO_OK" != "1" ]; then
  echo "REPRO_SELFCHECK_TIMEOUT" >&2
  exit 2
fi
...
case "`$RUNNING_SHA" in
  "`$SHA"|"`$SHORT") ;;
  *)
    # also accept first 12 of full sha
    SHORT12=`$(printf '%s' "`$SHA" | cut -c1-12)
    if [ "`$RUNNING_SHA" != "`$SHORT12" ]; then
      echo "SHA_MISMATCH expected=`$SHORT|`$SHA runtime=`$RUNNING_SHA" >&2
      exit 2
    fi
    ;;
esac
if [ "`$RUNNING_TAG" != "`$TAG" ]; then
  echo "TAG_MISMATCH expected=`$TAG runtime=`$RUNNING_TAG" >&2
  exit 2
fi
...
echo "SHIP_IMAGE_OK tag=`$TAG digest=`$DIGEST web=`$WEB REPRO_STATUS=PASS"
"@
