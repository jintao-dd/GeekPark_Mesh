...
def _seed(con):
    ...
    pub = json.dumps(
        {"title": "demo", "relations": [{"a": "A", "b": "B", "label": "接触"}]},
        ensure_ascii=False,
    )
    con.execute(
        ...
        (pub, json.dumps({"secret": "DRAFT_LEAK"}, ensure_ascii=False)),
    )
    con.execute(
        "INSERT INTO issues(slug, date_start, date_end, period_label, status, "
        "draft_json) VALUES "
...
def test_ask_published_one_tool(db_ready):
    r = _run("本周编辑部关注什么", feishu_open_id="ou_bound")
...
def test_empty_result_no_second_data_tool(db_ready):
    ...
    def empty_adapter(con, context, args):
        calls["n"] += 1
        return ToolResult(
            ok=True,
